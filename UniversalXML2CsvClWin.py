...
*This is the Command line version for windows
...
Contact me if these values are important or if
...
parser = argparse.ArgumentParser(description="""This is the Command line version. For Windows.
...
Comments from the xml file are not included in the CSV file.
Contact me if these values are important or if
there is an xml file that has crashed the program so I can fix bugs or add features.""", epilog="Here is an example: python UniversalXml2CsvCl.py /nw/hesm_nas/workdirs/dmohotti/newtransecttool/Indirforcsv/ /nw/hesm_nas/workdirs/dmohotti/newtransecttool/Outdirforcsv/ -m /nw/hesm_nas/projects/MWD_SouthDade/RSMGL_SouthDade/workdirs/rnovoa/RSMGL_SouthDade/COP/ECB19/main_ECB19.xml")


parser.add_argument("inDir", type=str, help="Input Directory path (place xml files here)")
parser.add_argument("outDir", type=str, help="Output Directory path (csv files will be placed here)")
parser.add_argument("-m","--mainXML", type=str, help="Optional mainXML file path. (This script will find all relative xml paths in this file and will create a csv version of that file)",default="")
args = parser.parse_args()


remove_digits = str.maketrans('', '', digits)

if path.exists(args.mainXML):
    dirname = os.path.dirname(args.mainXML)
...
for file in os.listdir(args.inDir):
    if file.endswith('.xml'):
        print('xmlfile',file)
        pathList.append(args.inDir + file)

txtpath = args.outDir + 'output.txt'
count = 0
#Get xml file paths
with open(txtpath, 'w') as text_file:
    if path.exists(args.mainXML):
        with open(args.mainXML) as fp:  
...
#                        print('eclist', eclist, file=text_file)       
                ...
                with open(args.outDir + filename + '.csv', 'w') as ofile:
                    ofile.write(title+'\n')
                    ofile.write(title2 +'\n')
